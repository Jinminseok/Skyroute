(() => {
    ...
    function initializeSeatSelection() {
        ...
        const legSections = Array.from(
            document.querySelectorAll(
                "[data-leg-section]"
            )
        );

        legSections.forEach(layoutSeatRows);
    ...
    function layoutSeatRows(section) {
        const wideBody =
            section.dataset.wideBody === "true";

        const seatRows = Array.from(
            section.querySelectorAll("[data-seat-row]")
        );

        seatRows.forEach(seatRow => {
            const container =
                seatRow.querySelector(".seat-row-groups");
            ...
            const seats = Array.from(
                container.querySelectorAll(".seat-button")
            );
            ...
            while (seatIndex < seats.length) {
                ...
                if (!lastBlock) {
                    ...
                    container.appendChild(lastBlock);
                }

                lastBlock.appendChild(
                    seats[seatIndex]
                );
